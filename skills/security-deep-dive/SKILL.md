...
- Authorization and access control: any endpoint or function that performs operations based on an object identifier (ID) from an untrusted source. The sink is the sensitive operation (read, update, delete) performed on that object. The danger is the absence of a check verifying that the object belongs to the current user's session or that the user has sufficient roles to perform the action (Broken Object Level Authorization / IDOR).
